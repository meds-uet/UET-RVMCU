...
module m1s2_slave_tb;
  ...
  // M1 requests bus access
  M1_HBUSREQ = 1;
  wait (M1_HGRANT == 1);
  $display("Time: %0t | M1_HGRANT received", $time);

  // Slave 1
  @(posedge HCLK);
  M1_HADDR = 32'h10000000; 
  M1_HTRANS = 2'b10;       
  M1_HSIZE = 3'b010;       
  M1_HBURST = 3'b000;      
  M1_HPROT = 4'b0011;
  ...
  @(posedge HCLK);  // Address Phase
  @(posedge HCLK);  // Data Phase
  if (M_HRDATA !== 32'hDEADBEEF) begin
      $display("ASSERTION FAILED: Expected 0xDEADBEEF, got %h", M_HRDATA);
      $fatal;
  end else begin
      $display("ASSERTION PASSED: Readback data = %h", M_HRDATA);
  end
