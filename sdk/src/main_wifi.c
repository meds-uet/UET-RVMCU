...
int main(void) {
   ...
   { 
    ...
    ////////////////////////////////////////////////////////////////////////////
    Uetrv32_Uart_Tx_String("AT\r\n");
    Uetrv32_Uart_Rx_String(string);
    
    Uetrv32_Uart_Tx_String("AT+RST\r\n");
    for (i = 0; i < 10; i++){
    Uetrv32_Uart_Rx_String(string);}
    ...
    Uetrv32_Uart_Tx_String("AT+CWMODE=2\r\n");
    for (i = 0; i < 10; i++) {
      Uetrv32_Uart_Rx_String(string);}
    ...
    Uetrv32_Uart_Tx_String("AT+CIPMUX=1\r\n");
    for (i = 0; i < 100; i++) {
      Uetrv32_Uart_Rx_String(string);}
      
    for (i = 0; i < 5000; i++) {}
    
    Uetrv32_Uart_Tx_String("AT+CWSAP=\"SSID\",\"password\",5,3\r\n");
    for (i = 0; i < 100; i++) {
      Uetrv32_Uart_Rx_String(string);}
    ...
    Uetrv32_Uart_Tx_String("AT+CIFSR\r\n");
    for (i = 0; i < 200; i++) {
      Uetrv32_Uart_Rx_String(string);}
      
    Uetrv32_Uart_Tx_String("AT+CIPSERVER=1,80\r\n");
    for (i = 0; i < 1000; i++){
      Uetrv32_Uart_Rx_String(string);}
      
    Uetrv32_Uart_Tx_String("AT+CIPSTART=\"TCP\",\"IP\",80\r\n");
    for (i = 0; i < 10000; i++){
      Uetrv32_Uart_Rx_String(string);}	
    
    Uetrv32_Uart_Tx_String("AT+CIPSTATUS\r\n");
    for (i = 0; i < 1000; i++){
      Uetrv32_Uart_Rx_String(string);}	
      
    for (i = 0; i < 500000; i++) {}
    
    Uetrv32_Uart_Tx_String("AT+CIPSEND=0,2\r\n"); 
    for (i = 0; i < 10; i++) {
      Uetrv32_Uart_Rx_String(string);}
        
    /*Uetrv32_Uart_Tx_String("AT+CIPSEND=0,5\r\n");
    for (i = 0; i < 10; i++) {
      Uetrv32_Uart_Rx_String(string);}
    ...
    Uetrv32_Uart_Tx_String("AT+CIPSEND=0,6\r\n");
    for (i = 0; i < 10; i++) {
      Uetrv32_Uart_Rx_String(string);}*/
